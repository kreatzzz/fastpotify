//! Draws Woofer's built-in Winamp skin: the one the skinned window
...
//! `assets/skins/woofer.wsz`, a classic skin in every respect (BMPs in
...
use woofer::skin::layout::{self, Area};
use woofer::skin::sprites::{self, Sheet, Sprite};
use woofer::skin::{Skin, font, zip};

type Rgb = [u8; 3];
/// A box on a canvas: x, y, width, height.
type Box = (i64, i64, i64, i64);

// Woofer's dark palette (src/theme.rs), so the window matches the app.
...
fn pledit_sheet() -> Canvas {
    ...
    // The left half's menus, painted and quiet: Woofer has no files
...
fn main() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("assets/skins/woofer.wsz");
    ...
    if args.next().as_deref() == Some("--preview") {
        ...
        let skin = Skin::from_archive("Woofer", &archive).expect("the skin just written reads");
