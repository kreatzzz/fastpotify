...
//! Woofer's own skin, a `.wsz` like any other that ships inside the
...
pub enum SkinError {
    ...
    #[error("this is a modern Winamp skin, which Woofer cannot draw; it needs a classic one")]
    ModernSkin,
...
impl Skin {
    ...
    /// The skin Woofer ships, drawn for it and packed as a `.wsz` like
...
const BUILTIN_ARCHIVE: &[u8] = include_bytes!("../../assets/skins/woofer.wsz");

static BUILTIN: LazyLock<Arc<Skin>> = LazyLock::new(|| {
    Arc::new(Skin::from_archive("Woofer", BUILTIN_ARCHIVE).expect("the built-in skin reads"))
});
...
mod tests {
    ...
    fn a_folder_of_bitmaps_is_a_skin_too() {
        let dir = std::env::temp_dir().join(format!("woofer-skin-{}", std::process::id()));
