...
#[cfg(not(target_os = "linux"))]
const DEVICES_REPLY: &str = "woofer:devices ";
...
mod tests {
    ...
    fn parses_every_control_verb() {
        ...
        assert!(matches!(parse("woofer:devices"), Some(Request::Devices)));
    }
    ...
    fn refuses_arguments_that_are_not_shaped_like_spotifys_own() {
        assert!(command("woofer:play-uri http://example.com/pwn").is_none());
        assert!(command("woofer:play-uri spotify:track:a b").is_none());
        assert!(command("woofer:play-uri ../../etc/passwd").is_none());
        assert!(command("woofer:play-uri").is_none());
        assert!(command(&format!("woofer:play-uri spotify:{}", "x".repeat(200))).is_none());
        assert!(command("woofer:transfer ../secrets").is_none());
        assert!(command("woofer:transfer").is_none());
