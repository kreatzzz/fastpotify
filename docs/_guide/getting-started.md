...
description: Install Woofer, sign in through your browser, and enable playback on this computer.
nav_order: 2
---
...
Woofer uses system fonts for scripts that its interface font does not
...
A desktop entry ships in `packaging/applications/woofer.desktop`.

## Sign in

Start the app and press **Sign in with Spotify**. Your browser opens
Spotify's own consent page; your password never touches Woofer. When
Spotify redirects back, your library loads and you can search, browse, and
control your other devices immediately.

Woofer stores a refresh token in your platform's state directory
(`~/.local/state/woofer` on Linux). You normally need the browser only
once per machine.
...
**Woofer** (rename it in Settings), visible from your phone like any
speaker.

## Basics

- **Closing the window does not stop the music.** Woofer keeps playing
