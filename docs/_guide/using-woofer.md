...
Woofer searches the local network when you open the device picker. It
...
Woofer requests lyrics from Spotify when local playback is authorized.
...
or **Switch to it** in Settings turns Woofer into a small player that
...
Drop a `.wsz` on either window and Woofer copies it into its skins
...
see-through. Quitting is in the right-click menu and Ctrl+Q. Woofer has no balance
...
Starting Woofer while it is already running brings the existing window
